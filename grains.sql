...
CREATE OR REPLACE PACKAGE grains# IS
   FUNCTION at_square(potencia NUMBER)
      RETURN NUMBER;
   FUNCTION total
      RETURN NUMBER;
END grains#;
...
CREATE OR REPLACE PACKAGE BODY grains# IS
   FUNCTION at_square(potencia NUMBER) RETURN NUMBER IS
   BEGIN
      return POWER(2, potencia - 1);
   END at_square;
   
   FUNCTION total RETURN NUMBER IS
     sumatorio NUMBER := 0;
   BEGIN
      FOR i IN 1..64 -1 LOOP
         sumatorio := sumatorio + POWER(2, i);
      END LOOP;
      RETURN 1+ sumatorio;
   END total;
END grains#;
