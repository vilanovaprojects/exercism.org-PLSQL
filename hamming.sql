CREATE OR REPLACE PACKAGE hamming# IS
...
CREATE OR REPLACE PACKAGE BODY hamming# IS
   FUNCTION distance(i_first VARCHAR2, i_second VARCHAR2)RETURN PLS_INTEGER IS
     diferencia pls_integer := 0;
   BEGIN
     IF LENGTH(i_first) = LENGTH(i_second) THEN
         FOR j IN 1..LENGTH(i_first) LOOP
            IF SUBSTR(i_first, j, 1) != SUBSTR(i_second, j, 1) THEN
