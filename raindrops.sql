create or replace package raindrops# is
   function convert(numero pls_integer)
   return varchar2;
end raindrops#;
/
...
create or replace package body raindrops# is
   function convert(numero pls_integer) return VARCHAR2 IS
      lavariable VARCHAR2(255);  
   begin
   IF numero MOD 3 = 0 THEN
      lavariable := 'Pling';
   END IF;
   IF numero MOD 5 = 0 THEN
      lavariable := lavariable || 'Plang';
   END IF;
   IF numero MOD 7 = 0 THEN
      lavariable := lavariable || 'Plong';
   END IF;
   IF lavariable IS NULL THEN
     lavariable := numero;
   END IF;
