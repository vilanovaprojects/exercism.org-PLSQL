CREATE OR REPLACE PACKAGE numeral# IS
    FUNCTION to_roman(numero VARCHAR2) RETURN VARCHAR2;
END numeral#;
/

CREATE OR REPLACE PACKAGE BODY numeral# IS
    FUNCTION to_roman(numero VARCHAR2) RETURN VARCHAR2 IS
        numerotemp  number(5) := 10000;
        digito      number(1);
        salida      varchar2(20);
        romannumerals   CHAR(9) := '  MDCLXVI';
        
    BEGIN
    IF numero > 3000 THEN
        dbms_output.put_line('No se permiten números mayores a 3000');
    ELSE
        numerotemp := numerotemp + numero;
        
        FOR j IN 1..4 LOOP
            digito := SUBSTR(numerotemp,(j+1),1);
            
            CASE
                WHEN digito < 4 THEN 
                    FOR i IN 1..digito LOOP
                        salida := salida || SUBSTR(romannumerals,(j*2)+1,1);
                    END LOOP;
                WHEN digito = 4 THEN 
                    salida := salida || SUBSTR(romannumerals,(j*2)+1,1) || SUBSTR(romannumerals,(j*2),1);
                WHEN digito < 9 THEN 
                    salida := salida || SUBSTR(romannumerals,(j*2),1);
                    FOR i IN 6..digito LOOP
                        salida := salida || SUBSTR(romannumerals,(j*2)+1,1);
                    END LOOP;
                WHEN digito = 9 THEN 
                    salida := salida || SUBSTR(romannumerals,(j*2)+1,1) || SUBSTR(romannumerals,(j*2)-1,1);
                ELSE
                dbms_output.put_line('error procesando el dígito ' || digito);
            END CASE;
        END LOOP;
    END IF;
